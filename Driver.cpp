...
void mj_function_setup( PDRIVER_OBJECT pdriver_object )
{
	...
	pdriver_object->MajorFunction[ IRP_MJ_CREATE ]		      = &major_functions::mj_create;
	pdriver_object->MajorFunction[ IRP_MJ_CLOSE ]		      = &major_functions::mj_close;
	pdriver_object->MajorFunction[ IRP_MJ_DEVICE_CONTROL ]    = &major_functions::mj_ioctl_dispatcher;
	util::log( "%s", "Major Function setup complete for KM READWRITE Driver finished." );	
}
...
extern "C" NTSTATUS DriverEntry( PDRIVER_OBJECT pdriver_object, UNICODE_STRING* )
{
	...
	const auto device_obj_creation_status = IoCreateDevice( 
		pdriver_object, 
		0, 
		&driver_globals::driver_device::device_name, 
		FILE_DEVICE_UNKNOWN, 
		FILE_DEVICE_SECURE_OPEN,
		false, 
		&driver_globals::driver_device::device_object 
	);
	...
	driver_globals::driver_device::device_object->Flags |= DO_DIRECT_IO;
	driver_globals::driver_device::device_object->Flags &= ~DO_DEVICE_INITIALIZING;

	const auto symbolic_link_creation_status = IoCreateSymbolicLink( &driver_globals::driver_device::dos_device_name, &driver_globals::driver_device::device_name );
